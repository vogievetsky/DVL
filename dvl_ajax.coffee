...
dvl.util = {
  stringObject: (obj) ->
    ...
    if type in ['object', 'array']
      ...
      keys.sort()
      str.push k, dvl.util.stringObject(obj[k]) for k in keys
      return str.join('|')
...
(->
  ...
  makeManager = ->
    ...
    getData = (resVal, status) ->
      ...
      if this.url is q.url.get()
        if not this.url
          ...
          q.resVal = resVal
          q.cache.store(this.url + this.dataStr, resVal) if this.url and q.cache

        else
          q.resVal = q.cache.retrieve(this.url)
    ...
    makeRequest = (q, request) ->
      ...
      url = q.url.get()
      data = q.data.get()
      dataStr = if data then dvl.util.stringObject(data) else ''
      method = q.method.get()
      ctx = { q, request, url, dataStr }
      if url?
        if q.cache?.has(url + dataStr)
          # load from cache
